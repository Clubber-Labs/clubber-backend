...
export async function postFeaturedEvent(
  ...
  request.log.info({ userId: request.user.sub, eventId: id, featureId: feature.id }, 'User added event to featured events')
...
export async function deleteFeaturedEvent(
  ...
  request.log.info({ userId: request.user.sub, eventId: id, featureId }, 'User removed event from featured events')
