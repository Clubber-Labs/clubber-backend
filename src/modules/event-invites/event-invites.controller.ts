...
export async function postInvite(request: FastifyRequest, reply: FastifyReply) {
  ...
  request.log.info({ userId: request.user.sub, eventId, invitedCount: result.count }, 'User invited others to event')
...
export async function getInvites(request: FastifyRequest, reply: FastifyReply) {
  ...
  request.log.info({ userId: request.user.sub, eventId }, 'User requested invites for event')
